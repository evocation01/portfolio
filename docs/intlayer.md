...
> Intlayer is compatible with Next.js 12, 13, 14, and 16. If you are using Next.js Page Router, you can refer to this [guide](/doc/environment/nextjs/next-with-page-router). For Next.js 12, 13, 14 with App Router, refer to this [guide](/doc/environment/nextjs/14).
...
> To Learn more about the `useIntlayer` hook, refer to the [documentation](/doc/packages/next-intlayer/useIntlayer).
...
> The `intlayerProxy` is used to detect the user's preferred locale and redirect them to the appropriate URL as specified in the [configuration](/doc/concept/configuration). Additionally, it enables saving the user's preferred locale in a cookie.
...
export const LocaleSwitcher: FC = () => {
    ...
    return (
        <div>
            <button popoverTarget="localePopover">
            ...
            <div id="localePopover" popover="auto">
                {availableLocales.map((localeItem) => (
                    <Link
                        ...
                        key={localeItem}
                        aria-current={
                            locale === localeItem ? "page" : undefined
                        }
...
> An alternative way is to use the `setLocale` function provided by the `useLocale` hook. This function will not allow prefetching the page. See the [`useLocale` hook documentation](/doc/packages/next-intlayer/useLocale) for more details.
...
>
> -   [`useLocale` hook](/doc/packages/next-intlayer/useLocale)
> -   [`getLocaleName` hook](/doc/packages/intlayer/getLocaleName)
> -   [`getLocalizedUrl` hook](/doc/packages/intlayer/getLocalizedUrl)
> -   [`getHTMLTextDir` hook](/doc/packages/intlayer/getHTMLTextDir)
